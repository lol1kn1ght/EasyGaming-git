    ...
    try {
      ...
      var temproles =
        filtred_timed_roles
          ?.map(
            (val) =>
              `<@&${val.role}>: ${f.time(val.time - new Date().getTime())}`
          )
          .join("\n") || "Список пуст";
      ...
      let others = `\`Текущее\`: ${
        profile_data.bans?.length || "0"
      } 🔨   |    ${profile_data.mutes?.length || "0"} 🔇   |   ${
        profile_data?.warns?.length || "0"
      } ⚠️\n\`Архив\`: ${member_archieve?.bans?.length || 0} 🔨  | ${
        member_archieve?.mutes?.length || 0
      } 🔇  | ${member_archieve?.warns?.length || 0} ⚠️`;
      ...
      this.interaction.editReply({ embeds: [repMsg] });
    } catch (err) {
