...
class Command extends Command_template {
  constructor(args, interaction) {
    ...
    this.options = {
      ...
      slash: {
        name: "bans",
        description: "Вывести список блокировок человека. [Смотритель+]",
        options: [
          {
            name: "упоминание",
            description: "ИЛИ Упоминание участника",
            type: 6,
            required: false,
          },
          {
            name: "айди",
            description: "ИЛИ Айди участника",
            type: 3,
            required: false,
          },
        ],
    ...
    try {
      ...
      if (!warns[0])
        return this.msgFalseH(
          `У пользователя \`${member?.user?.tag || member_id}\` нет блокировок.`
        );
      ...
      for (let warn of warns) {
        ...
        let text = `\n\`\`\`js\nМодератор: ${
          ...
          time === 0 ? "Навсегда" : f.time(warn.time)
        }\n\`\`\`\n`;
        ...
        embeds.push(
          new Discord.MessageEmbed({
            ...
            author: {
              ...
              name: `🔨 Список блокировок - ${member?.user.tag || member_id}`,
            },
            description: page,
            footer: {
